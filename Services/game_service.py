...
class GameService:
    ...
    def join_game(self, game_id: int, player_name: str) -> bool:
        ...
        battlefield = Battlefield(-100,100,-100,100,-100,100)
    ...
    def get_game_status(self, game_id: int, shooter_name: str) -> str:
        ...
        for player in game.get_players():
            if player.get_name() == shooter_name:
                if all(vessel.get_hit()==0 for vessel in player.get_battlefield().get_vessels())==True or all(vessel.get_weapon().get_ammunitions()==0 for vessel in player.get_battlefield().get_vessels())==True:
                    return "PERDU"
            elif all(vessel.get_hit()==0 for vessel in player.get_battlefield().get_vessels())==True or all(vessel.get_weapon().get_ammunitions()==0 for vessel in player.get_battlefield().get_vessels())==True:
                return "GAGNE"
            else:
                return "ENCOURS"
